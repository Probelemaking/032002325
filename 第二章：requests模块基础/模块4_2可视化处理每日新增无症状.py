...
def get_data():
    ...
    file_name = '中国每日本土新增无症状人数（转置版）.xlsx'
    df = pd.read_excel(file_name)
    ...
    map_chart = (
        ...
        .set_global_opts(
            title_opts=opts.TitleOpts(
                title="" + str(day) + "中国每日本土新增无症状人数(单位:人） 数据来源：国家卫健委",
                subtitle="",
                pos_left="center",
                pos_top="top",
...
def draw_new_asymptomatic():
    ...
    timeline.render(date+"月份中国每日本土新增新无症状人数（可视化界面）.html ")
    print("已生成"+date+"月份中国每日本土新增无症状人数（可视化界面）.html！！")
    print(date+"月份动态可视化大屏生成成功,请点击查看！！!")


if __name__ == '__main__':
    draw_new_asymptomatic()
